...
class User(Base):
    ...
    uuid: Mapped[str] = mapped_column(String(50), unique=True, init=False, default_factory=uuid4_str)
    phone: Mapped[str] = mapped_column(String(11), unique=True, index=True, nullable=False, comment="手机号")
    username: Mapped[str] = mapped_column(String(20), unique=True, index=True, comment="用户名")
    password: Mapped[str | None] = mapped_column(String(255), default=None, comment="密码")
    nickname: Mapped[str | None] = mapped_column(String(20), default=None, comment="昵称")
    email: Mapped[str | None] = mapped_column(String(100), default=None, comment="邮箱")
    avatar: Mapped[HttpUrl | None] = mapped_column(String(255), default=None, comment="头像")
    gender: Mapped[int | None] = mapped_column(INTEGER, default=None, comment="性别(0未知 1男 2女)")
    ...
    is_admin: Mapped[bool] = mapped_column(Boolean, default=False, comment="超级权限(0否 1是)")
    is_verified: Mapped[bool] = mapped_column(Boolean, default=False, comment="是否实名认证")
    salt: Mapped[bytes | None] = mapped_column(VARBINARY(255), default=None, comment="加密盐")
