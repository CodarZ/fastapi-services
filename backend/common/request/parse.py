...
from backend.core.paths import Ip2RegionPath
...
def get_location_offline(ip: str) -> dict | None:
    ...
    try:
        cb = XdbSearcher.loadContentFromFile(dbfile=Ip2RegionPath)
...
def parse_user_agent_info(request: Request) -> UserAgentInfo:
    ...
    # 如果 User-Agent 为空，直接返回默认的 UserAgentInfo 对象
    if not user_agent:
    ...
    os_version = (
        f"{_user_agent.os.version_string}" if _user_agent.os.version_string else None
    )

    # 获取浏览器及版本
    browser = _user_agent.browser.family
    browser_version = (
        f"{_user_agent.browser.version_string}"
        if _user_agent.browser.version_string
        else None
    )
